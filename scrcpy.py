...
class ScrcpyRunner:
    ...
    def _detect_scrcpy(self) -> bool:
        ...
        # Check system PATH for scrcpy first,
        system_scrcpy = shutil.which('scrcpy')
        if system_scrcpy:
            scrcpy_paths.append(Path(system_scrcpy))

        # Add dependencies/scrcpy/scrcpy if current system is linux
        if platform.system() == 'Linux':
            scrcpy_paths.append(Path(__file__).parent / 'dependencies' / 'scrcpy' / 'scrcpy')

        # Add dependencies/scrcpy/apple/scrcpy if current system is apple
        if platform.system() == 'Darwin':
            scrcpy_paths.append(Path(__file__).parent / 'dependencies' / 'scrcpy' / 'apple' / 'scrcpy')
